...
BranchPredictor* BranchPredictor::create(core_id_t core_id)
{
   try
   {
      config::Config *cfg = Sim()->getCfg(); // ? cfg？ namespace config中 class Config的一个指针
      assert(cfg);

      m_mispredict_penalty = cfg->getIntArray("perf_model/branch_predictor/mispredict_penalty", core_id);  //求penalty
...
void BranchPredictor::updateCounters(bool predicted, bool actual)
{
   ...
   printf("called BranchPredictor %d %d \n ",m_correct_predictions,m_incorrect_predictions);
}
