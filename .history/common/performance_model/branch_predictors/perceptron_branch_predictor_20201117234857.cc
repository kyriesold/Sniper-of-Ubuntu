...
bool PerceptronBranchPredictor::predict(IntPtr ip, IntPtr target){
    for(int i=0;i<num_of_history;i++){
        result+=weight[size][i+1]*globalhistory[size][i]+weight[size][0];
    }
    return result>0;
}
...
void PerceptronBranchPredictor::update(bool predicted, bool actual, IntPtr ip, IntPtr target)
{
    ...
    globalhistory[index].push_back(actual);
    globalhistory[index].erase(globalhistory.begin()+1);
    if(predicted!=actual||result<threshold){
        for(int i=1;i<num_of_history+1;i++){
            weight[index][i]+=globalhistory[index][i-1]*actual;
        }
    }    
}