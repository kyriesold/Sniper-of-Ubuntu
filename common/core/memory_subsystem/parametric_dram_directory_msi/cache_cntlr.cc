...
CacheCntlr::processMemOpFromCore(
      ...
      Core::mem_op_t mem_op_type,    //mem操作类型 一般为LOAD or STORE
...
LOG_ASSERT_ERROR(offset + data_length <= getCacheBlockSize(), "access until %u > %u", offset + data_length, getCacheBlockSize());
   ...
   //通过这个operationPermissibleinCache可以得到cache是否hit！！！！！
   bool cache_hit = operationPermissibleinCache(ca_address, mem_op_type, &cache_block_info), prefetch_hit = false;

   if (!cache_hit && m_perfect)  //cache未命中 且 m_perfect->??
   {
      cache_hit = true;
      hit_where = HitWhere::where_t(m_mem_component);
      if (cache_block_info)  //cache_block_info非空
         cache_block_info->setCState(CacheState::MODIFIED);  //这是修改为脏位吗？标记为已修改
      else    //else  插入
      {
   ...
   }                                        //m_passthrough 为 false 说明上级缓存不支持缓存直通
   else if (cache_hit && m_passthrough)  //LOG_ASSERT_ERROR(m_passthrough == false, "Cache pass-through not supported on last-level cache")
   {
      cache_hit = false;
      cache_block_info->invalidate();  //使得cache_block_info无效化
...
MYLOG("processMemOpFromCore l%d before next", m_mem_component);   //进入到next-level进行操作 返回hit or miss
...
MYLOG("processMemOpFromCore l%d after next fill", m_mem_component);
      ...
      SubsecondTime t_now = getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_USER_THREAD);
      copyDataFromNextLevel(mem_op_type, ca_address, modeled, t_now); //去下层cache里面拿数据

      cache_block_info = getCacheBlockInfo(ca_address);  //拿到  cache_block_info的数据
   ...
   //在这个地方访问cache，hit_where决定是否进行update_replacement
...
MYLOG("copyDataFromNextLevel l%d", m_mem_component);
   ...
   if (modeled && !m_next_level_read_bandwidth.isInfinite())
   {
      ...
      LOG_ASSERT_ERROR(t_done >= t_now + delay, "Did not expect next-level cache to be this fast");  //L2 Cache中没那么快
...
CacheCntlr::trainPrefetcher(IntPtr address, bool cache_hit, bool prefetch_hit, SubsecondTime t_issue)
{
   ...
   {
      //printf("called trainPrefetcher->if \n");
      ...
      {
         //printf("called trainPrefetcher->if->for \n");
...
CacheCntlr::Prefetch(SubsecondTime t_now)
{
   ...
   //string sss = Marshal.PtrToStringAnsi(address_to_prefetch);
   //printf("called Perfetch %s \n"，sss);
...
CacheCntlr::doPrefetch(IntPtr prefetch_address, SubsecondTime t_start)
{
   ...
   HitWhere::where_t hit_where = processShmemReqFromPrevCache(this, Core::READ, prefetch_address, true, true, Prefetch::OWN, t_start, false);//进入到next-level进行操作 返回hit or miss
      ...
      IntPtr address, Core::mem_op_t mem_op_type, CacheBlockInfo **cache_block_info)
{
   ...
   bool cache_hit = false;   //默认为一个cache miss
   CacheState::cstate_t cstate = getCacheState(block_info);  //I guess cache是否hit是根据这个block_info是否能够匹配上 得来的

   switch (mem_op_type)
   {
      case Core::READ:
         cache_hit = CacheState(cstate).readable();  //根据cstate返回cache是否hit
         break;
...
{
   //printf("called getCacheBlockInfo");
...
void
CacheCntlr::incrementQBSLookupCost()  //记录延迟时间
{
