...
{
   //count++;
   //printf("called read_line %X %X %X %X %X %X \n",line_index,offset,&out_buff,bytes,update_replacement,count);
   assert(offset + bytes <= m_blocksize);
   //assert((out_buff == NULL) == (bytes == 0));

   if (out_buff != NULL && m_blocks != NULL) //m_blocks = new char[m_associativity * m_blocksize]  表示组内的整个block的size
      memcpy((void*) out_buff, &m_blocks[line_index * m_blocksize + offset], bytes);   //out_buff应该是要读取的数据，从cache block中复制走，达到LOAD的目的
      //line_index * m_blocksize确定是那个line，offset确定是line内那个byte，bytes应该是确定是哪几个bytes？
...
{
   //count++;
   //printf("called write_line %X %X %X %X %X %X \n",line_index,offset,&in_buff,bytes,update_replacement,count);
   assert(offset + bytes <= m_blocksize);
...
{
   for (SInt32 index = m_associativity-1; index >= 0; index--)   //从后往前找
   {
      if (m_cache_block_info_array[index]->getTag() == tag)    //如果tag匹配
      {
         if (line_index != NULL)
            *line_index = index;    //记录行号
         return (m_cache_block_info_array[index]);    //返回这个cacheblock的信息（tag和组内index）
      }
   }
   return NULL;
}
...
{
   //printf("called parsePolicyType");   for many times
