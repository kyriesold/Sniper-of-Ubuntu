...
{ 
   IntPtr addr;
   CacheBase::access_t access_type;
   Byte* buff;
   UInt32 bytes;
   SubsecondTime now;
  ...
  void set(IntPtr addr, CacheBase::access_t access_type,  Byte* buff, UInt32 bytes, SubsecondTime now, bool update_replacement)
  {
          this->addr=addr;
         ...
         this->bytes=bytes;
         this->now=now;
         this->update_replacement=update_replacement;
  }
 };
//************************************
      ...
      Byte* buff, UInt32 bytes, SubsecondTime now, bool update_replacement)
{
   ...
   splitAddress(addr, tag, set_index, block_offset);//addr右移6位得到tag
   
   
   CacheSet* set = m_sets[set_index];
   CacheBlockInfo* cache_block_info = set->find(tag, &line_index);  //cache_block_info是在组内用tag（内存地址）去匹配，找到cache block的信息cache_block_info
   ...
   if (access_type == LOAD)  //判断访问类型是LOAD还是STORE
   {
      // NOTE: assumes error occurs in memory. If we want to model bus errors, insert the error into buff instead
      if (m_fault_injector)   //将错误error插入到总线bus中？
      ...
      SubsecondTime now, CacheCntlr *cntlr)
{
   ...
   //printf("called insertSingleLine %X \n",set_index);
   m_sets[set_index]->insert(cache_block_info, fill_buff,//插入cache block 在这里
...
Cache::updateCounters(bool cache_hit)
{
   ...
   //printf("called updateCounters %X \n",m_num_hits);
}
