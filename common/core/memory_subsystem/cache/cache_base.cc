...
:      //：冒号应该表示要从CacheBase类中继承的东西
   ...
   m_ahl(ahl)
{
   m_log_blocksize = floorLog2(m_blocksize);     //继承了共性之后还得在class里面加上自己的特性
   m_log_num_sets = floorLog2(m_num_sets);        //算出num_sets和blocksize这两个参数的对数值
...
// utilities
CacheBase::hash_t
CacheBase::parseAddressHash(String hash_name)   //解析HASH地址 返回相应的HASH NAME
{
   if (hash_name == "mask")
...
CacheBase::splitAddress(const IntPtr addr, IntPtr& tag, UInt32& set_index) const
{
   ...
   //printf("called splitAddress %X %X %X \n",addr,tag,set_index);
}
