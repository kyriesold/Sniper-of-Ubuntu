...
void TraceThread::handleInstructionDetailed(Sift::Instruction &inst, Sift::Instruction &next_inst, PerformanceModel *prfmdl)
{
   ...
   // Set up instruction
   int a[4]={0x0F,0xAE,0x3b,0};    // 3rd  {72,61,1,240}   1st {184,1}
   //printf("%#x ",(inst.sinst->data[0]));
  // printf("%#x ",(inst.sinst->data[1]));
   //printf("%#x ",(inst.sinst->data[2]));   //0x6c
   //printf("%#x ",(inst.sinst->data[3]));    //0x24
                                              //0x4c
